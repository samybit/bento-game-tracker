...
// Accept the new isExpanded prop
export default function GameGrid({ games, isExpanded = false }: { games: Game[], isExpanded?: boolean }) {
  ...
  return (
    // Adjust column count based on expansion state
    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 h-full overflow-y-auto custom-scrollbar pr-2 pb-4 content-start ${isExpanded ? 'lg:grid-cols-3 xl:grid-cols-4' : 'xl:grid-cols-2 2xl:grid-cols-3'}`}>
      {games.map((game) => {
        ...
        return (
          // Remove inner padding on the card if expanded so the image is flush to the edges
          <div key={game.id} className={`bg-[#0a0a0a] border border-[#262626] rounded-xl flex flex-col hover:border-gray-600 transition-colors ${isExpanded ? 'h-[400px]' : 'h-[350px] p-5'}`}>
